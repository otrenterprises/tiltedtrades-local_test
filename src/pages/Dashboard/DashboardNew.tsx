...
export function DashboardNew({ calculationMethod, showGrossPL }: DashboardProps) {
    ...
    return (
      <PageLayout title="Dashboard">
        <div className="text-center text-red-400 p-8">
          ...
          <p className="text-sm text-primary0 mt-2">Please check your connection and try again.</p>
        </div>
      </PageLayout>
    )
  }
  ...
  if (!metrics) {
    return (
      <PageLayout title="Dashboard">
        <div className="text-center text-tertiary">Calculating metrics...</div>
      </PageLayout>
    )
  }
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
          <div className="bg-secondary border border-theme rounded-lg p-6">
            <h3 className="text-lg font-semibold text-primary mb-4">Commission Analysis</h3>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-tertiary">Total Commission</span>
                <span className="text-sm font-semibold text-loss">{formatCurrency(metrics.totalCommission)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-tertiary">Gross P&L</span>
              ...
              <div className="flex justify-between items-center pt-2 border-t border-theme">
                <span className="text-sm text-tertiary">Net P&L</span>
                <span className={`text-lg font-bold ${metrics.totalPL >= 0 ? 'text-profit' : 'text-loss'}`}>
                  {formatCurrency(metrics.totalPL)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs text-primary0">Avg Commission/Trade</span>
          ...
          <div className="bg-secondary border border-theme rounded-lg p-6">
            <h3 className="text-lg font-semibold text-primary mb-4">Trade Breakdown</h3>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-tertiary">Total Trades</span>
                <span className="text-sm font-semibold text-secondary">{metrics.totalTrades}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-tertiary">Winning Trades</span>
                <span className="text-sm font-semibold text-profit">{metrics.winningTrades} ({formatPercentage(metrics.winRate)})</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-tertiary">Losing Trades</span>
                <span className="text-sm font-semibold text-loss">{metrics.losingTrades}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-tertiary">Breakeven Trades</span>
                <span className="text-sm font-semibold text-tertiary">{metrics.breakevenTrades}</span>
              </div>
            </div>
          </div>
        </div>
